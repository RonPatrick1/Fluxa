...
object BatteryVoiceSettings {
    ...
    const val DEFAULT_SPEECH_TEMPLATE = "Battery {battery} percent."
    ...
    fun speechTemplate(context: Context): String =
        preferences(context).getString(KEY_SPEECH_TEMPLATE, DEFAULT_SPEECH_TEMPLATE)
            ?.trim()
    ...
    fun renderSpeech(context: Context, speaker: FamilySpeaker, level: Int): String =
        speechTemplate(context)
            .replace("{speaker}", speaker.name)
            .replace("{battery}", level.coerceIn(0, 100).toString())
            .replace("{device}", deviceLabel(context))
}
